(function () {
    ...
    angular
        .module('WeServices')
        .provider('RegionService', function () {
            var cityJson = [
            ...
            var getProvince = function () {
                return _.filter(cityJson,
                    function (val) {
                        return (val.code.substr(2, 4) == '0000');
                    });
            };

            var getCities = function (provinceCode) {
                var prefix0_2 = provinceCode.substr(0, 2);
                return _.filter(cityJson,
                    function (val) {
                        return val.code.substr(0, 2) == prefix0_2 && val.code.substr(2, 4) != '0000' && val.code.substr(4, 2) == '00'
                    });
            };

            var getAreas = function (cityCode) {
                return _.filter(cityJson,
                    function (val) {
                        if (cityCode == '110100' || cityCode == "120100" || cityCode == "310100" || cityCode == "500100") {
                            return val.code.substr(0, 3) == cityCode.substr(0, 3) && val.code.substr(4, 2) != '00';
                        } else {
                            return val.code.substr(0, 4) == cityCode.substr(0, 4) && val.code.substr(4, 2) != '00';
                        }
                    });
            };
            
            var getAllCities = function() {
            	return _.filter(cityJson, function(item) {
            		if (item.code != '110100' || item.code != "120100" || item.code != "310100" || item.code != "500100") {
            			if (item.code.substr(2, 2) == '00') {
            				return item.code.substr(2, 2) != '00';
            			} else {
            				return item.code.substr(4, 2) == '00';
            			}
            		}
            	})
            };
        ...
        .provider('CityService', function () {
            var cityIndexs = 'ABCDEFGHJKLMNPQRSTWXYZ';
            var hotCities = ['北京', '上海', '广州', '深圳', '成都', '西安', '天津', '武汉', '重庆', '南京'];
            var cityArray = new Array(
                ['鞍山', '安庆', '安阳', '安康'],
                ['北京', '保定', '包头', '巴彦淖尔', '本溪', '蚌埠', '亳州', '滨州', '宝鸡', '巴音郭楞'],
